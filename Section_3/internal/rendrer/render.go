...
func CreateTempleteCache() (map[string]*template.Template, error) {
	...
	pages, err := filepath.Glob("/home/ihtgoot/web_dev/Section_3/templates/*-page.tpml")
	if err != nil {
	...
	for _, page := range pages {
		...
		//base page
		matche, err := filepath.Glob("/home/ihtgoot/web_dev/Section_3/templates/*-layout.tpml")
		if err != nil {
			return Cacahe, err
		}
		// use base
		if len(matche) > 0 {
			templateSet, err = templateSet.ParseGlob("/home/ihtgoot/web_dev/Section_3/templates/*layout.tpml")
			if err != nil {
				return Cacahe, err
			}
		}
